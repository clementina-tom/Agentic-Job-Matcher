...
def metrics() -> dict[str, Any]:
    ...
    fit_scores = [float(m.get("fit_score", 0.0)) for m in matches] if matches else []
    avg_fit = sum(fit_scores) / len(fit_scores) if fit_scores else 0.0

    statuses = Counter(row.get("status", "unknown") for row in applications)
    source_types = Counter((row.get("job") or {}).get("source_type", "unknown") for row in matches)
    ...
    return {
        ...
        "quality": {
            ...
            "high_fit_count": len([m for m in matches if float(m.get("fit_score", 0.0)) >= 0.68]),
        },
        "application_status_breakdown": dict(statuses),
